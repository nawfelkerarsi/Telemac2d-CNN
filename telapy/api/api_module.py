...
class ApiModule():
    ...
    def __init__(self, name, casfile,
        ...
        @param name (string) Name of the code (t2d, t3d, ...)
        ...
        try:
            ...
            raise TelemacException(
                ...
                f'_api{machinery.EXTENSION_SUFFIXES[0]}\n'
    ...
    def partitionning_step(self, cas, code=None):
        ...
        @param code (str) Code name for partel (T2D, T3D...)
        """
        if code is None:
    ...
    def concatenation_step(self, cas, code=None):
        ...
        @param code (str) Code name for gretel (T2D, T3D...)
        """
    ...
    def run_one_time_step_rst(self):
        """
        Run one time step only writing in restart part
        """
        if self.name != 't2d':
            raise TelemacException("Function run_one_time_step_rst only "
                                   "available for t2d")
        ...
        self._error = self._run_timestep_rst(self.my_id)
    ...
    def mpi_get(self, varname, i=-1, root=-1):
        ...
        if local_i != -1:
            # Checking if we have an interface point
            # in that case we are only fetching the value for the one with the
            # lowest rank
            fetch = True
            if local_i in self._nachb[:, 0]:
                for val in self._nachb:
                    if val[0] == local_i:
                        fetch = val[1] > self.rank
    ...
    def mpi_set(self, varname, value, i=-1, root=-1):
        """
        Set the value of a variable using global numbering
