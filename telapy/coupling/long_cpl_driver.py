...
class LongCplDriver:
    ...
    def exec_creation(self):
        ...
        if os.path.isdir(os.path.join(self.case_path, 'EXEC')):
            rmtree(os.path.join(self.case_path, 'EXEC'), ignore_errors=True)
    ...
    def preparation_model(self):
        ...
        if self.param['reprise']:
            ...
            if not os.path.exists(file_tmp):
                ...
                if os.path.exists(path_tmp):
                    os.remove(path_tmp)
                path_ori = f'WaterLineInit_{tstart}.slf'
                orislf = TelemacFile(os.path.join(folder_cas, path_ori))
                if orislf.fformat != 'SERAFIND':
                    print(f'Update precision of initial restart file {os.path.basename(path_ori)}')
                    alter(os.path.join(folder_cas, path_ori), path_tmp,
                          toggle_precision=True)
                    print('')
                else:
                    if platform == 'win32':
                        copy2(path_ori, path_tmp)
                    else:
                        os.symlink(path_ori, path_tmp)
                del orislf
        ...
        cas.set('RESTART MODE', True)
        cas.set('RESTART FILE', 'T2DRST')
        cas.set('RESTART FILE FORMAT', 'SERAFIND')
        cas.set('RESTART FILE PRINTOUT PERIOD', 1)
    ...
    def __call__(self, config_run=None):
        ...
        while tdeb < tfinal:
            ...
            filedst = os.path.join(self.case_path,
                                   ...
                                   result_file)
            if self.param['chkPts'] or tdeb + freq == tfinal:
                adj = 'final' if tdeb + freq == tfinal else 'stored'
                print(f'Thinning down times in {adj} checkpoint file {os.path.basename(restart_file)}')
                alter('T2DRST', 'T2DRST_END', bnd_file='T2DCLI',
                      tfrom=-1, force=True)
                file_mv("T2DRST_END", restart_file)
                print('')
            ...
            if os.path.isdir(os.path.join(self.case_path, 'EXEC',
                             '__pycache__')):
                rmtree(os.path.join(self.case_path, 'EXEC', '__pycache__'), ignore_errors=True)
