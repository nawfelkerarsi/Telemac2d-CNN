...
class ClassCpl2D:
    ...
    def node_frliq(self, id_fr_co_2d):
        ...
        self.tab_frliq = []
        self.nb_node_loc = np.zeros(self.nb_fr_co, dtype=int)
        self.nb_node_glo = np.zeros(self.nb_fr_co, dtype=int)
        ...
        if self.has_fr_co:
            self.tab_frliq = np.array(self.tab_frliq, dtype=object)
    ...
    def del_duplic_halo_pt(self, idx, nachb):
        ...
        @param idx (int) coupling boundary index
    ...
    def get_node_frco(self, idx, nptfr, numliq, nbor_numliq):
        ...
        @param idx (int) coupling boundary index
        @param nptfr (int) number of boundary points
        @param numliq (list) list of liquid border number
        @param nbor_numliq (list) border and 2d numbering correspondence
        """
        compteur_g = 0
        tab_node_frliq_tmp = []
        tmp2 = []
    ...
    def compt_type_cl(type_cl_in_1d):
        ...
        @return ncote (int) nb of water level conditions
    ...
    def calc_sl_moy(self, glo_h, i=0):
        ...
        @param i (int) number of 1D model
