...
class Gebco(Selafin):
    ...
    def __init__(self, fname, vals=(None, None)):
        ...
        ikle3 = - np.ones((2*(nx1d-1)*(ny1d-1), 3), dtype=int)
        ...
        # ~~> final connectivity
        self.ikle3 = - np.ones_like(gikle, dtype=int)
        ...
        print('     +> mesh x,y,z')
        # ~~> defines grid
        x = xllcorner + xdim * np.arange(nx1d, dtype=float) - xdim / 2.
        y = yllcorner - ydim * np.arange(ny1d, dtype=float) + \
            ydim * ny1d - ydim / 2.
        ...
        print('        - set')
        self.ipob3 = np.zeros(self.npoin3, dtype=int)
        iptfr = 0
