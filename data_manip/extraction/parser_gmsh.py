...
class MSH(Selafin):
    ...
    def __init__(self, file_name):
        ...
        # ~~ Section Nodes ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            if key == "Nodes":
                ...
                if self.fle['float'][0] == 'd':
                    ...
                    meshx = np.zeros(npoin, dtype=float)
                    meshy = np.zeros(npoin, dtype=float)
                    meshz = np.zeros(npoin, dtype=float)
                # map_nodes = []
                for i in range(npoin):
                    line = fle.next().split()
                    # map_nodes.append(int(line[0]))
                    meshx[i] = float(line[1])
                    meshy[i] = float(line[2])
                    meshz[i] = float(line[3])
        ...
        # ~~ Section Nodes ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                ...
                nelem = int(fle.next())
                ikle2 = - np.ones((nelem, 3), dtype=int)
                for i in range(nelem):
                    line = fle.next().split()
                    if int(line[1]) != 2:
                        continue
                    expr = line[int(line[2])+3:]
                    ikle2[i] = [int(expr[0]), int(expr[1]), int(expr[2])]
        ...
        self.ipob3 = np.ones(self.npoin3, dtype=int)
