...
from data_manip.extraction.linspace import curvilinear_abscissa, linspace_poly
from data_manip.formats.serafin_file import SerafinFile
from telapy.api.hermes import HermesFile, elem2str, ndp2elem
...
class TelemacFile(HermesFile if HERMES_AVAIL else SerafinFile):
    ...
    def nplan(self):
        ...
        if self._nplan is None:
            self._nplan = max(0, self.get_mesh_nplan())
    ...
    def get_interpolator(self, values, interp_method="matplotlib.LinearTri"):
        """
        ...
        if interp_method == "scipy.LinearND":
            grid_pt = np.column_stack(
                (self.meshx[: self.npoin2], self.meshy[: self.npoin2])
            )
            return interpolate.LinearNDInterpolator(grid_pt, values)
        elif interp_method == "matplotlib.LinearTri":
            interpolator = mtri.LinearTriInterpolator(self.tri, values)
            # Resolve a Matplotlib issue related to a circular dependency
            # between Matplotlib.Triangulation and Matplotlib.Trifinder. The
            # easiest way consists in removing a useless reference to the
            # parent triangulation instance in TriFinder constructor, since
            # is is never used.
            trifinder = self.tri.get_trifinder()
            if hasattr(trifinder, "_triangulation"):
                delattr(trifinder, "_triangulation")

            return interpolator
        else:
            raise TelemacException(
                f"Unknown interpolation method: {interp_method}\n"
                "Available methods: matplotlib.LinearTri, scipy.LinearND"
            )
    ...
    def interpolate_mesh_on_points(self, points, values,
        ...
        @param values (np.ndarray) Values for each mesh point.
        @param method (str) Interpolation method to use.
        """
        data_interp = self.get_interpolator(values, interp_method)

        if interp_method == "scipy.LinearND":
            pts = np.asarray([(pt[0], pt[1]) for pt in points])
            return data_interp(pts)

        # Default to matplolib.LinearTri
    ...
    def get_data_on_horizontal_slice(
        ...
        if isinstance(zslices, (list, np.ndarray)):
            ...
            raise TelemacException(f'Unknown zslices type: {type(zslices)}')
    ...
    def read_mesh(self, src=None):
        ...
        # nptfr and ipob3 are read regardless of the presence of a boundary file
        # as they are needed in the SELAFIN format
