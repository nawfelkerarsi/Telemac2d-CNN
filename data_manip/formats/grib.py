...
class Grib(object):
    ...
    def set_geometry(self):
        ...
        # ~~> Boundaries
        self.slf2d.ipob3 = np.zeros(self.slf2d.npoin3, dtype=int)
    ...
    def put_geometry(self, file_name):
        ...
        spec = np.zeros((nb_direct, nb_freq, self.nx1d, self.ny1d),
                        dtype=float)
        var = np.zeros((self.nx1d, self.ny1d), dtype=float)
    ...
    def set_spectral(self):
        ...
        for grb in grbs:
            ...
            self.freq = np.asarray(grb.scaledFrequencies, dtype=float) / \
                grb.frequencyScalingFactor
            #  /!? only so that TOMAWAC works
            self.dirc = np.asarray(grb.scaledDirections, dtype=float) / \
        ...
        self.slf2d.meshx = np.zeros(self.slf2d.npoin2, dtype=float)
        self.slf2d.meshy = np.zeros(self.slf2d.npoin2, dtype=float)
        print('      - set the mesh')
        ipoin = 0
