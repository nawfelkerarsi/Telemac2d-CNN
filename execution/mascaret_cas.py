...
class MascaretCas():
    ...
    def __init__(self,
        ...
        self.root_file = ''
        if len(self.file_name.split(sep)) > 1:
            for i in range(len(self.file_name.split(sep))-1):
                self.root_file += self.file_name.split(sep)[i] + sep
        else:
            self.root_file = '.' + sep
    ...
    def _set_io_files_xcas_cas(self):
		...
		#TODO: inversed key comparing above ==> change
                ...
                xlim_courlis = cas.get("FICHIER POUR LES COTES D'EROSION IMPOSEES")
                self.in_files[xlim_courlis] = 'xlim'
            ...
            if len(self.out_files) != 0:
                val = self.get('parametresGeneraux/optionCourlis')
                if val is not None:
                    casfile = self.get('parametresGeneraux/fichierMotCleCourlis')
                    dicofile = path.join(CFGS.get_root(), "sources", "mascaret", "data",
                                     "dico_Courlis.txt")
                    casfile = self.root_file + casfile
                    cas = TelemacCas(casfile, dicofile)
                    self.in_files['FICHIER DE GEOMETRIE COURLIS'] = cas.get('FICHIER DE GEOMETRIE COURLIS')
                    self.out_files['listing_courlis'] = cas.get('FICHIER LISTING COURLIS')
