...
import shutil
from os import chdir, getcwd, listdir, mkdir, path, remove

from config import CFGS
from data_manip.computation.amp2wave import amp2wave
from utils.exceptions import TelemacException
from utils.files import (get_file_content, is_newer, put_file_content,
                         symlink_file)
...
def process_lit(cas, cas_dir, ncsize, tmp_dir, use_link):
    ...
    for key in cas.in_files:
        ...
        if path.exists(tmp_file_name):
            if is_newer(tmp_file_name, file_name):
