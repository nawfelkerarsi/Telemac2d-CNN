    ...
    for k in cas.out_files:
        ...
        if tpe[0:7] == 'HISTORY':
            ...
            try:
                ...
                nplan = 0
            run_grehis(gretel, file_name, file_format, glogeo, fmtgeo,
                       globnd, ncsize, nplan, method)
        if tpe[0:6] == 'DELWAQ':
            print('     collecting: ' + path.basename(file_name))
...
def run_grehis(grehis, grehis_file, file_format, geom, geo_format, bnd,
               ncsize, nplan, method=1, bypass=False):

    """
    @brief Runs GREHIS, the functions that merges results

    @param grehis (string): the path of grehis
    @param grehis_file (string): the name of the GREHIS file
    @param file_format (string): format of the GREHIS file
    @param geom (string): name of the geometry file
    @param geo_format (string): format of the geometry file
    @param bnd (string): name of the boundary file
    @param ncsize (int): number of processors
    ...
    # ~~ Change GRETEL into GREHIS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    path_grehis = path.dirname(grehis)
    _, ext_grehis = path.splitext(path.basename(grehis))
    grehis = path.join(path_grehis,
                       'grehis_autop' + ext_grehis)
    # ~~ Run GREHIS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    grehis_par = 'grehis_'+grehis_file+'.par'
    grehis_log = 'grehis_'+grehis_file+'.log'
    put_file_content(grehis_par,
                     [geom, geo_format, bnd, grehis_file,
                      file_format, str(ncsize), str(nplan),
                      str(method)])
    mes = Messages(size=10)
    cmd = '{} < {} >> {}'.format(grehis, grehis_par, grehis_log)
    print('     +> '+cmd)
    tail, code = mes.run_cmd(cmd, bypass)
    if code != 0:
        if path.exists(grehis_log):
            log = "Here is the log:\n" + \
                  '\n'.join(get_file_content(grehis_log))
        else:
            log = "No log available check command:\n"+cmd
        raise TelemacException(
            'Could not merge your file '+grehis_file
            + ' (runcode='+str(code)+') with the error as follows:'
            + '\n        '+tail+'\n\n'+log)
