...
"""
from os import chdir, getcwd, mkdir, path, sep
from shutil import rmtree
from time import localtime, strftime

from config import CFGS
from execution.get import (get_glogeo, get_gretel_cmd, get_hpc_cmd,
                           get_hpc_depend, get_mpi_cmd, get_partel_cmd,
                           get_partitionner)
from execution.mascaret_cas import MascaretCas
from execution.process import (check_para_tilling, process_artnim,
                               process_config, process_ecr, process_executable,
                               process_lit)
from execution.run import run_code, run_partition, run_recollection
...
from utils.files import get_file_content, put_file_content, zipsortie
    ...
    def __init__(self, steering_file, code_name, working_dir, mascaret_cas=False):
        ...
        if self.code_name == 'mascaret':
            ...
            if self.cas.file_type == 'cas':
                ...
                val = self.cas.get('parametresGeneraux/optionCourlis')
                if val is not None:
                    casfile = self.cas.get('parametresGeneraux/fichierMotCleCourlis')
                    dicofile = path.join(CFGS.get_root(), "sources", "mascaret", "data",
                                     "dico_Courlis.txt")
                    self.cpl_cases['courlis'] = TelemacCas(casfile,dicofile)
